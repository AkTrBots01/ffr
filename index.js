...
async function starts() {
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `FOI BOM TE CONHECER, VOLTE LOGO PRO @${num.split('@')[0]}👋`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			   })
    })
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			const ownerNumber = ["558896668696@s.whatsapp.net"] // COLOQUE SEU NÚMERO AQUI SEM ESPAÇOS
			...
			case 'tagall':
				...
				case 'bc':
					...
					if ((args[0]) === 'on') {
						...
						reply(`\`\`\`✓Ativado com sucesso o modo antiracismo no grupo, Agora Fala Algo Que Envolva Racismo Com Alguém Ai Membros Comuns .\`\`\` *${groupMetadata.subject}*`)
					} else if ((args[0]) === 'off') {
